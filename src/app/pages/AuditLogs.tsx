...
export function AuditLogs() {
  ...
  const fetchUsers = () => {
    ...
    if (storedUsers) {
      ...
      // Fallback for default admin if no users key exists yet
      setUsers([{ email: "admin@example.com", name: "Admin User" }]);
    }
  };
  ...
  // Apply search + status + user filter
  const filteredLogs = tabLogs.filter((log) => {
    ...
    const matchesUser = selectedUsers.size === 0 || selectedUsers.has(log.user);
  ...
  // Helper functions for user filter
  const toggleUserFilter = (userEmail: string) => {
  ...
  // Get all unique users from logs plus registered users
  const allUserOptions = Array.from(new Set([
    ...users.map(u => u.email),
    ...logs.map(l => l.user),
  ])).sort();
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-64 p-0" align="end" onInteractOutside={(e) => e.preventDefault()}>
            ...
            <div className="max-h-64 overflow-y-auto p-2">
              {allUserOptions.length === 0 ? (
                ...
                allUserOptions.map((userEmail) => {
                  const userInfo = users.find((u) => u.email === userEmail);
                  const displayName = userInfo ? `${userInfo.name} (${userEmail})` : userEmail;
                  const isSelected = selectedUsers.has(userEmail);
                  ...
                  return (
                    <div
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        toggleUserFilter(userEmail);
                      }}
                      className="flex items-center gap-2 px-3 py-2 rounded-md hover:bg-gray-50 cursor-pointer transition-colors"
                    >
                      <Checkbox
                        checked={isSelected}
          ...
          {Array.from(selectedUsers).map((userEmail) => {
            ...
            return (
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1 bg-indigo-50 border border-indigo-200 text-indigo-700 rounded-full text-sm hover:bg-indigo-100 transition-colors"
              >
